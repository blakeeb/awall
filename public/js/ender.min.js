...
/*!
    ...
    * MIT License
    */
(function(name,definition,context){typeof module!="undefined"&&module.exports?module.exports=definition():typeof context["define"]=="function"&&context.define.amd?define(name,definition):context[name]=definition()})("qwery",function(){function toArray(ar){return[].slice.call(ar,0)}function isNode(el){var t;return el&&typeof el=="object"&&(t=el.nodeType)&&(t==1||t==9)}function uniq(ar){var a=[],i,j;e:for(i=0;i<ar.length;i++){for(j=0;j<a.length;j++)if(a[j]==ar[i])continue e;a[a.length]=ar[i]}return a}function qwery(selector,opt_root){var m,root=typeof opt_root=="string"?qwery(opt_root)[0]:opt_root||doc;root=isFinite(root.length)&&root[0]&&!root.nodeName?root[0]:root;return!root||!selector?[]:doc.getElementsByClassName&&selector=="string"&&(m=selector.match(classOnly))?toArray(root.getElementsByClassName(m[1])):selector&&(selector.document||selector.nodeType&&selector.nodeType==9)?opt_root?[]:[selector]:isNode(selector)?!opt_root||isAncestor(selector,root)?[selector]:[]:toArray(root.querySelectorAll(selector))}var classOnly=/^\.([\w\-]+)$/,doc=document,win=window,html=doc.documentElement,isAncestor="compareDocumentPosition"in html?function(element,container){return(container.compareDocumentPosition(element)&16)==16}:function(element,container){container=container==doc||container==window?html:container;return container!==element&&container.contains(element)};qwery.uniq=uniq;return qwery},this);provide("qwery-mobile",module.exports);(function($){var q=function(){var r;try{r=require("qwery")}catch(ex){r=require("qwery-mobile")}finally{return r}}();$.pseudos=q.pseudos;$._select=function(s,r){return($._select=function(){var b;if(typeof $.create=="function")return function(s,r){return/^\s*</.test(s)?$.create(s,r):q(s,r)};try{b=require("bonzo");return function(s,r){return/^\s*</.test(s)?b.create(s,r):q(s,r)}}catch(e){}return q}())(s,r)};$.ender({find:function(s){var r=[],i,l,j,k,els;for(i=0,l=this.length;i<l;i++){els=q(s,this[i]);for(j=0,k=els.length;j<k;j++)r.push(els[j])}return $(q.uniq(r))},and:function(s){var plus=$(s);for(var i=this.length,j=0,l=this.length+plus.length;i<l;i++,j++)this[i]=plus[j];this.length+=plus.length;return this},is:function(s,r){var i,l;for(i=0,l=this.length;i<l;i++)if(q.is(this[i],s,r))return!0;return!1}},!0)})(ender)})()